...
class TenantService(CommonService):
    ...
    def get_joined_tenants_by_user_id(cls, user_id):
        fields = [
            cls.model.id.alias("tenant_id"),  # 租户ID，并重命名为tenant_id
            cls.model.name,  # 租户名称
            cls.model.llm_id,  # 大语言模型ID
            cls.model.embd_id,  # 嵌入模型ID
            cls.model.asr_id,  # 语音识别模型ID
            cls.model.img2txt_id,  # 图像转文本模型ID
            UserTenant.role,  # 用户在租户中的角色
        ]
        return list(
            cls.model.select(*fields)
            .join(
                UserTenant,
                on=(
                    (cls.model.id == UserTenant.tenant_id)  # 租户ID关联
                    & (UserTenant.user_id == user_id)  # 指定用户
                    & (UserTenant.status == StatusEnum.VALID.value)  # 有效状态
                    & (UserTenant.role == UserTenantRole.NORMAL)  # 普通用户角色
                ),
            )
            .where(cls.model.status == StatusEnum.VALID.value)
            .dicts()  # 转换为字典格式
