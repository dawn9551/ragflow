...
class LoggerFactory(object):
    ...
    def get_handler(class_name, level=None, log_dir=None, log_type=None, job_id=None):
        if not log_type:
            ...
            log_file = os.path.join(
                log_dir,
                "rag_flow_{}.log".format(log_type)
                if level == LoggerFactory.LEVEL
                else "rag_flow_{}_error.log".format(log_type),
            )
...
def base_msg(
    ...
    task=None,
    role: str = None,
    party_id: typing.Union[str, int] = None,
    detail=None,
):
    if detail:
        detail_msg = f" detail: \n{detail}"
    ...
    if task is not None:
        return (
            f"task {task.f_task_id} {task.f_task_version} ",
            f" on {task.f_role} {task.f_party_id}{detail_msg}",
        )
